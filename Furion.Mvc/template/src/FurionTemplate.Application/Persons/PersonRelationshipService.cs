...
namespace FurionTemplate.Application.Persons
{
    ...
    public class PersonRelationshipService : IDynamicApiController
    {
        ...
        public PersonRelationshipService(IRepository<Person> personRepository, IRepository<V_Person> repository)
        {
        ...
        {
            var persons = await _personRepository.Include(u => u.PersonDetail, false).ToListAsync();
        ...
        public async Task<List<PersonDto>> OneToMany()
        {
            var persons = await _personRepository.Include(u => u.PersonDetail, false)
                .Include(u => u.Childrens).ToListAsync();
            return persons.Adapt<List<PersonDto>>();
        }
